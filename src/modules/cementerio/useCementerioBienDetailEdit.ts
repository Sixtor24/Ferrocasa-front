...
export type CementerioBienDetailDraft = {
  descripcion: string;
...
function draftFromBien(bien: BienMueble): CementerioBienDetailDraft {
  return {
...
function draftsEqual(a: CementerioBienDetailDraft, b: CementerioBienDetailDraft) {
  return (
...
function almacenOptionsForCementerio(almacenes: ApiAlmacen[]) {
  const catalogNames = new Set(ALMACENES_CEMENTERIO.map(normalizeCatalogValue));
...
type UseCementerioBienDetailEditParams = {
  bien: BienMueble;
...
export function useCementerioBienDetailEdit({
  bien,
  almacenes,
  disabled = false,
  onSaved,
}: UseCementerioBienDetailEditParams) {
  ...
  const almacenOptions = useMemo(
    () => almacenOptionsForCementerio(almacenes),
    [almacenes],
  );
  ...
  const patchDraft = (patch: Partial<CementerioBienDetailDraft>) => {
