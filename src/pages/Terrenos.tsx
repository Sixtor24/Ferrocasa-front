...
export default function Terrenos() {
  ...
  const terreno = id ? terrenos.find((t) => t.id === Number(id)) : null;
  const protos = terreno ? protocolizaciones.filter((p) => p.terrenoId === terreno.id) : [];
  ...
  const filtered = useMemo(() => {
    return terrenos.filter((t) => {
      ...
      if (filtros.zonificacion && t.zonificacion !== filtros.zonificacion) return false;
      if (filtros.nroPropiedad && !t.nroPropiedad.includes(filtros.nroPropiedad)) return false;
      if (filtros.levantamiento && t.levantamientoTopografico !== filtros.levantamiento) return false;
      if (filtros.acreditacion && t.acreditacionTecnicaAmbiental !== filtros.acreditacion) return false;
      if (q && !`${t.codigo} ${t.identificacion} ${t.ubicacion}`.toLowerCase().includes(q)) return false;
      return true;
    });
  }, [filtros]);
